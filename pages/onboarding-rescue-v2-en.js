...
export default function LeadRescueOnboardingEN() {
  ...
  const validateForm = () => {
    ...
    // Required fields
    if (!form.businessName) errs.push('Business name required');
    if (!form.ownerName) errs.push('Owner name required');
    if (!form.ownerEmail) errs.push('Owner email required');
    if (!form.ownerMobile) errs.push('Owner mobile phone required');
    if (!form.businessPhone) errs.push('Business phone required');
    if (!form.inboxEmail) errs.push('Inbox email required');
    if (!form.serviceArea) errs.push('Service area required');
    if (!form.jobTypesWanted) errs.push('Wanted job types required');
    if (!form.hoursWeek) errs.push('Weekday hours required');

    // HARD GATE: Call forwarding
    if (form.canEnableCallForwarding === '') {
      errs.push('You must indicate if you can enable call forwarding');
    } else if (form.canEnableCallForwarding === 'no') {
      errs.push('BLOCKING: Call forwarding after X rings is REQUIRED for the Lead Rescue system. Please contact your telecom provider and return when it\'s enabled.');
    } else if (!form.callForwardingAck) {
      errs.push('You must confirm that you understand how call forwarding works');
    }

    // Required acknowledgments
    if (!form.smsConsentAck) errs.push('You must confirm SMS/MMS consent');
    if (!form.smsOptOutAck) errs.push('You must confirm adding an opt-out line');
    if (!form.noPricingAck) errs.push('You must confirm that the AI will never quote prices');
    if (!form.noTimingAck) errs.push('You must confirm that the AI will never guarantee timelines');
    ...
    if ((form.dailySummaryDelivery === 'email' || form.dailySummaryDelivery === 'both') && !form.dailySummaryEmail) {
      errs.push('Email for daily summary required');
    }
    ...
    if ((form.dailySummaryDelivery === 'sms' || form.dailySummaryDelivery === 'both') && !form.dailySummarySMS) {
      errs.push('SMS number for daily summary required');
    }

    // Booking link if using booking method
    if (form.nextStepPreference === 'booking' && !form.bookingLink) {
      errs.push('Booking link required if using this method');
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus('✅ Submitted! I will analyze your information and prepare your Lead Rescue system. You will receive an email within 24h with next steps.');
    } catch (error) {
      console.error(error);
        ...
        <section className="max-w-4xl mx-auto space-y-10 px-6 sm:px-12 py-10 rounded-3xl bg-slate-950/80 border border-white/10 backdrop-blur-md shadow-[0_0_45px_rgba(15,23,42,0.9)]">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4 border border-blue-500/30 rounded-2xl p-6 bg-slate-900/60">
              <h2 className="text-xl font-semibold text-blue-300">A. Basic Information *</h2>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium">Owner Name / contact principal *</label>
                  <input
                    name="ownerName"
                ...
                <div>
                  <label className="block text-sm font-medium">Owner Mobile Phone *</label>
                  <input
                    type="tel"
                    name="ownerMobile"
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="yes">Yes, je peux l'activer</option>
                    <option value="no">No, impossible</option>
                  </select>
                </div>
              </div>
                ...
                <>
                  ...
                  <div className="flex items-start gap-2 bg-blue-950/30 p-4 rounded-xl">
                    ...
                    <label className="text-sm">
                      <strong>I confirm:</strong> Calls will ring normally on my main number. If no one answers after {form.ringSeconds} seconds, the call will be forwarded to an AI number that will answer automatically.
                    </label>
                  </div>
                </>
              )}
            ...
            <div className="space-y-4 border border-slate-700/70 rounded-2xl p-6 bg-slate-900/60">
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2 bg-blue-950/30 p-4 rounded-xl">
                  ...
                  />
                  <label className="text-sm">
                    <strong>I confirm:</strong> The AI will only send SMS/MMS to people who have contacted us (missed call or incoming SMS). Canadian anti-spam law compliance.
                  </label>
                </div>
                ...
                <div className="flex items-start gap-2 bg-blue-950/30 p-4 rounded-xl">
                  ...
                  />
                  <label className="text-sm">
                    <strong>I confirm:</strong> We will include an opt-out line (e.g., "Reply STOP to stop receiving messages").
                  </label>
                </div>
            ...
            <div className="space-y-4 border border-slate-700/70 rounded-2xl p-6 bg-slate-900/60">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    required
                    placeholder="Ex: Repair HVAC, installation, urgences"
                    className="mt-1 w-full rounded-xl px-4 py-2 text-slate-900 bg-white/95"
                  />
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium">Residential / Commercial *</label>
                  <select
            ...
            <div className="space-y-4 border border-amber-500/50 rounded-2xl p-6 bg-amber-950/20">
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2 bg-amber-950/30 p-4 rounded-xl">
                  ...
                  />
                  <label className="text-sm">
                    <strong>I confirm:</strong> The AI will NEVER quote prices. It will always say "A technician will contact you for an assessment and accurate price."
                  </label>
                </div>
                ...
                <div className="flex items-start gap-2 bg-amber-950/30 p-4 rounded-xl">
                  ...
                  />
                  <label className="text-sm">
                    <strong>I confirm:</strong> The AI will NEVER guarantee specific timelines. It will say "within 24-48h" or "based on availability", never "in 2 hours".
                  </label>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    ...
                    <option value="both">Both (courriel + SMS)</option>
                  </select>
                </div>
            ...
            <div className="space-y-4 border border-slate-700/70 rounded-2xl p-6 bg-slate-900/60">
              <h2 className="text-xl font-semibold text-slate-200">I. Hours of Operation *</h2>
            ...
            {isPlumbing && (
              <div className="space-y-4 border border-slate-700/70 rounded-2xl p-4 bg-slate-900/60">
                <h2 className="text-lg font-semibold">Détails Plumbing (optionnel)</h2>

                <div>
                  <label className="block text-sm font-medium">Domaines principaux</label>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                    {['Residential', 'Commercial', 'Débouchage', 'Égout / conduite principale', 'Water Heaters', 'Rénos cuisine / salle de bain'].map((area) => (
            ...
            {isRoofing && (
              <div className="space-y-4 border border-slate-700/70 rounded-2xl p-4 bg-slate-900/60">
                <h2 className="text-lg font-semibold">Détails Roofing (optionnel)</h2>
            ...
            {isElectrician && (
              <div className="space-y-4 border border-slate-700/70 rounded-2xl p-4 bg-slate-900/60">
                <h2 className="text-lg font-semibold">Détails Electrician (optionnel)</h2>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium">Emergency 24/7</label>
                    <select
            ...
            {isOther && (
              <div className="space-y-4 border border-slate-700/70 rounded-2xl p-4 bg-slate-900/60">
                <h2 className="text-lg font-semibold">Détails — Other métier (optionnel)</h2>
                ...
                <div>
                  <label className="block text-sm font-medium">Emergencys acceptées ?</label>
                  <select
                    name="otherEmergency"
