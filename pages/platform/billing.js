...
export default function BillingPage() {
  ...
  if (loading) {
    return (
      <DashboardLayout>
        <div className="flex items-center justify-center h-64 text-d-text0">Loading billing...</div>
      </DashboardLayout>
    );
  }
    ...
    return (
      <DashboardLayout>
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-center h-48 text-d-text0">No subscription data available for this customer.</div>
        </div>
      </DashboardLayout>
    );
  }
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-d-muted hover:text-d-text transition-colors"
            >
        ...
        <div className={`rounded-xl border ${style.border} ${style.bg} p-6`}>
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="text-xs text-d-text0">Revenue Share</div>
          ...
          {sub.last_paid_at && (
            <div className="mt-3 text-xs text-d-text0">Last paid: {fmtDate(sub.last_paid_at)}</div>
          )}
          {sub.suspended_at && (
        ...
        {isAdmin && preview && (
          <div className="p-6 rounded-xl border">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <div className="text-xs text-d-text0">Gross Revenue</div>
                <div className="text-lg font-bold text-d-text">{fmt(preview.grossRevenue)}</div>
              </div>
              <div>
                <div className="text-xs text-d-text0">Base Fee</div>
                <div className="text-lg font-bold text-d-text">{fmt(preview.baseFee)}</div>
              </div>
              <div>
                <div className="text-xs text-d-text0">Performance Fee ({(preview.rate * 100).toFixed(0)}%)</div>
                <div className="text-lg font-bold text-d-text">{fmt(preview.performanceFee)}</div>
              </div>
              <div>
          ...
          <div className="overflow-hidden rounded-xl border">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-d-border text-d-text0 text-xs">
                ...
                <tbody>
                  {data.invoices.map((inv) => (
                    <tr key={inv.id} className="border-b border-d-border/50 hover:bg-d-surface/30">
                      <td className="px-6 py-3 font-mono text-xs text-d-muted">{inv.invoice_number}</td>
                      <td className="px-6 py-3 text-d-muted text-xs">
                        {fmtDate(inv.period_start)} — {fmtDate(inv.period_end)}
                      </td>
                      <td className="px-6 py-3 text-right text-d-muted">{fmt(inv.gross_revenue)}</td>
                      <td className="px-6 py-3 text-right text-d-muted">{fmt(inv.base_fee)}</td>
                      <td className="px-6 py-3 text-right text-d-muted">{fmt(inv.performance_fee)}</td>
                      <td className="px-6 py-3 text-right font-semibold text-d-text">{fmt(inv.amount_due)}</td>
                      <td className="px-6 py-3 text-center">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${INVOICE_STYLES[inv.status] || ""}`}>
                          {inv.status}
                        </span>
                      </td>
                      <td className="px-6 py-3 text-xs text-d-muted">{fmtDate(inv.paid_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {isAdmin && data.events?.length > 0 && (
          <div className="overflow-hidden rounded-xl border">
            ...
            <div className="divide-y divide-d-border/50 max-h-64 overflow-y-auto">
              {data.events.map((evt) => (
                <div key={evt.id} className="px-6 py-3 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-d-text0">{fmtDate(evt.created_at)}</span>
                    <span className="text-d-muted font-medium">{evt.event_type}</span>
                    {evt.actor && <span className="text-d-text0">by {evt.actor}</span>}
                  </div>
                </div>
              ))}
            </div>
...
function CustomerSelector({ customers, selectedId, onChange }) {
  return (
    <div className="relative">
      ...
      <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-d-muted pointer-events-none" />
    </div>
  );
}
