...
export function mapQuoteToQboInvoice(quote, opts = {}) {
  ...
  // DocNumber is intentionally NOT set: QBO returns "Invalid Number" when
  // "Custom Transaction Numbers" preference is OFF (sandbox default). Pass
  // opts.allowDocNumber=true once we've confirmed the realm has the setting
  // enabled (P02 will read Preferences first).
  if (opts.allowDocNumber && quote.quote_number) {
