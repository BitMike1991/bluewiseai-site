 ...
 * @param {object} opts  — {
 *   customerRefValue: string (QBO Customer.Id),
 *   taxCodeRef?: string         // backward-compat: applied to every line if set
 *   combinedTaxCodeId?: string  // P02 — QBO TaxCode.Id for TPS+TVQ combined; sets per-line TaxCodeRef + GlobalTaxCalculation:"TaxExcluded"
 *   exemptTaxCodeId?: string    // optional, used when quote tax totals are 0
 *   allowDocNumber?: boolean
 * }
...
export function mapQuoteToQboInvoice(quote, opts = {}) {
  ...
  // Resolve the per-line TaxCodeRef. Priority:
  //   1. Explicit opts.taxCodeRef (back-compat / one-off)
  //   2. opts.combinedTaxCodeId (P02 TPS+TVQ when quote has positive tax)
  //   3. opts.exemptTaxCodeId   (P02 fallback for $0-tax quotes)
  //   4. omit (QBO uses customer default)
  const lineTaxCodeId =
    opts.taxCodeRef ??
    (taxGst + taxQst > 0 ? opts.combinedTaxCodeId : opts.exemptTaxCodeId) ??
    null;

  // QBO validates `Amount == Qty * UnitPrice` with zero tolerance (error 6070,
  // "Amount calculation incorrect"). We compute UnitPrice first (rounded to
  // 2 decimals) and DERIVE Amount = Qty * UnitPrice so the equality holds.
  // Any sub-cent drift from the prorata is absorbed on the final line by
  // nudging UnitPrice (preserving the invariant by construction).
  const Lines = rawLines.map((l, idx) => {
    const scaledUnitPrice = QBO_ROUND(l.unit_price * scale);
    const amount = QBO_ROUND(scaledUnitPrice * l.qty);
    const detail = { Qty: l.qty, UnitPrice: scaledUnitPrice };
    if (lineTaxCodeId) detail.TaxCodeRef = { value: String(lineTaxCodeId) };
    return {
      Id: String(idx + 1),
      LineNum: idx + 1,
  ...
  // If prorata scaling caused sum(Lines) to drift from subtotal by rounding,
  // adjust the LAST line's UnitPrice by the exact cent delta (keeping Qty
  // unchanged). Since QBO re-derives Amount = Qty * UnitPrice, we must shift
  // UnitPrice — not Amount — to stay consistent with error 6070.
  if (scale !== 1 && Lines.length > 0) {
    const sumLines = Lines.reduce((s, L) => s + L.Amount, 0);
    const drift = QBO_ROUND(subtotal - sumLines);
    if (Math.abs(drift) >= 0.01) {
      const last = Lines[Lines.length - 1];
      const newUnit = QBO_ROUND(last.SalesItemLineDetail.UnitPrice + drift / last.SalesItemLineDetail.Qty);
      last.SalesItemLineDetail.UnitPrice = newUnit;
      last.Amount = QBO_ROUND(newUnit * last.SalesItemLineDetail.Qty);
    }
  }
  ...
  const payload = {
    ...
    PrivateNote: `BW quote id=${quote.id} · ref=${quote.quote_number || "-"}`,
    Line: Lines,
  };
  if (opts.allowDocNumber !== false && quote.quote_number) {
    payload.DocNumber = String(quote.quote_number).slice(0, 21);
  }
  // Canada Invoice MUST set GlobalTaxCalculation when per-line TaxCodeRef is
  // used, otherwise QBO won't add tax on top of net Line.Amount values. US
  // realms accept the field too — it's a no-op there.
  if (lineTaxCodeId) {
