...
function renderItemCard(item, index, priceDisplayMode) {
  ...
  // Specs as bullet list or plain text
  let specsHtml = '';
  if (specs) {
    if (Array.isArray(specs)) {
      specsHtml = `<ul class="specs">${specs.map(s => `<li>${escHtml(String(s))}</li>`).join('')}</ul>`;
    } else {
      // Comma-separated string → split into list items
  ...
  const dimsHtml = (w && h) ? `<div class="dims">${escHtml(String(w))}<span class="x"> × </span>${escHtml(String(h))}</div>` : '';

  // Price line
  const installNoteHtml = (priceDisplayMode !== 'total')
...
export function generatePurQuoteHtml(data, config) {
  ...
  if (fenetreItems.length > 0) {
    ...
    itemsHtml += `<div class="item-grid">${fenetreItems.map((item, i) => renderItemCard(item, i + 1, priceDisplayMode)).join('\n')}</div>`;
  }
  if (porteItems.length > 0) {
    ...
    itemsHtml += `<div class="item-grid">${porteItems.map((item, i) => renderItemCard(item, porteOffset + i + 1, priceDisplayMode)).join('\n')}</div>`;
  }
  if (fenetreItems.length === 0 && porteItems.length === 0 && fenItems.length > 0) {
    // Fallback: no categorization possible
    itemsHtml += `<div class="item-grid">${fenItems.map((item, i) => renderItemCard(item, i + 1, priceDisplayMode)).join('\n')}</div>`;
  }
