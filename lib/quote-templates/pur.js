...
// Human labels for material + color ids persisted on line items.
const MATERIAL_LABEL_FR = { upvc: 'uPVC', hybride: 'Hybride (uPVC + aluminium)' };
const COLOR_LABEL_FR = {
  blanc: 'Blanc',
  noir: 'Noir',
  anodise: 'Anodisé',
  charbon: 'Charbon',
  brun_commercial: 'Brun commercial',
  sur_mesure: 'Couleur sur mesure',
};

function materialLabel(id) { return MATERIAL_LABEL_FR[id] || id || ''; }
function colorLabel(id)    { return COLOR_LABEL_FR[id]    || id || ''; }

/**
 * Build the human "Matériau · couleurs" spec string for an item from its
 * persisted material/color_ext/color_int fields. Returns '' when nothing is set
 * so legacy items without the fields render unchanged.
 */
function buildMaterialColorSpec(item) {
  if (!item || (!item.material && !item.color_ext && !item.color_int)) return '';
  const mat = materialLabel(item.material);
  const ext = colorLabel(item.color_ext);
  const int = colorLabel(item.color_int || item.color_ext);
  if (!mat && !ext) return '';
  const parts = [];
  if (mat) parts.push(mat);
  if (ext) {
    if (int && int !== ext) parts.push(`ext. ${ext} · int. ${int}`);
    else parts.push(`couleur ${ext}`);
  }
  return parts.join(' · ');
}
...
function renderItemCard(item, index, priceDisplayMode, complexityMul = 1) {
  ...
  // ENERGY STAR: Mikael 2026-04-22 "le devis devrait aussi mentionner energie
  // star sur tout les produits". All fenêtres, portes, patios default ON;
  // only an explicit item.energy_star === false (or item.energy === false)
  // opts the line out.
  const energy = !(item.energy_star === false || item.energy === false);
  const cert = item.cert || null;
  // Per-item note ("Évier", "Cuisine", "Porte arrière"...). Top-level field
  // is the source of truth; legacy line_items only carry it inside the specs
  // string as "Note: X" — fall back to that so old quotes also show it.
  const noteFromSpecs = (() => {
  ...
  // Specs as bullet list — Mikael 2026-04-22: identical condensed format as
  // the BDC (collection, profilé cadre+couleur, couleur intérieure,
  // épaisseur totale, egress, thermos, configuration, moustiquaire). One
  // source of truth in lib/bons-de-commande/template.js. ENERGY STAR line
  // is dropped here because we already show it as a pill via `energyBadge`
  // below.
  ...
  // Positional two-digit index on the item circle. The per-item SKU
  // (item.sku) is preserved in the database for internal supplier reference
  // but intentionally NOT rendered on the client-facing devis — Mikael S163
  // rule: project_ref (PUR-0042) is the visible reference; the number
  // inside each card is just positional.
  const itemNumLabel = String(index).padStart(2, '0');

  return `
    <div class="item-card">
      <div class="item-num">${itemNumLabel}</div>
...
export function generatePurQuoteHtml(data, config) {
  ...
  // Human-facing project reference (PUR-0042). Preferred over the random
  // quote_number token which is only used internally for the /q/[token] URL.
  // Falls back to quote_number for legacy quotes without a project_ref.
  const docNumEsc = escHtml(project_ref || quote_number);
  ...
  if (notesTemplate || notes) {
    ...
    if (notes && notes !== notesTemplate) {
      // F-010 — `notes` comes from the operator's free-text field in
      // DevisEditor and is rendered on the public /q/[token] page.
      // defaultNotes + notesTemplate are server-authored trusted HTML
      // (contain intentional <strong> tags), so only the user path is
      // escaped.
      notesItems.push(escHtml(notes));
    }
  } else {
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media print {
      ...
      /* break-inside:avoid is unreliable on CSS Grid items (Chrome/Safari
         print ignores it) — switch the item grid to block so each card is a
         plain block-level element that respects page-break-inside:avoid. */
      .item-grid {
        display: block !important;
        gap: 0;
        margin-bottom: 14px;
      }
      /* Same fix for the Parties au devis grid — the CLIENT card was splitting
         across pages 1-2 with its header orphaned. */
      .coord-grid {
        display: block !important;
        gap: 0;
      }
      ...
      /* Dimensions must stay on one line — "47 1/2 × 30 1/4" was wrapping at
         the × on narrow sketch columns. */
      .item-sketch .dims {
        font-size: 9px;
        white-space: nowrap;
      }
      ...
      /* Sections: allow breaks between items but keep the section title tied to
         the next card so we never leave a title orphaned at page bottom. */
      .section { break-inside: auto; }
      .section-title { break-after: avoid; break-inside: avoid; }
      .category-title { break-after: avoid; }
      /* Forfait + payment blocks shouldn't split */
      .forfait-section, .payment-list, .interac-note { break-inside: avoid; page-break-inside: avoid; }
      .subtotal-box, .install-box, .promo-box { break-inside: avoid; }
