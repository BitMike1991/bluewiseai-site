...
function renderItemCard(item, index, priceDisplayMode, complexityMul = 1) {
  ...
  // Prepend material + colors when present so they appear first in the list.
  const matColor = buildMaterialColorSpec(item);
  let specsHtml = '';
  {
    const existing = specs ? dedupeSpecs(specs) : [];
    const parts = matColor ? [matColor, ...existing] : existing;
    if (parts.length > 0) {
