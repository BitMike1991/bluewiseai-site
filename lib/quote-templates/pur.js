...
function renderItemCard(item, index, priceDisplayMode, complexityMul = 1) {
  ...
  // SKU badge: Jérémy's custom item reference (SKU-01, SKU-02…). Falls back
  // to the positional two-digit index when not set so legacy quotes still
  // render a number.
  const itemNumLabel = item.sku ? escHtml(item.sku) : String(index).padStart(2, '0');
