...
export function renderWindowConfigSvg(type, config, width = 100, height = 75) {
  ...
  if (type === 'guillotine') {
    ...
    panels.forEach((p, i) => {
      ...
      content += panelSliding({ type: p, x, y: pad, w: panelW, h: innerH });
      x += panelW;
    });
...
/**
 * Correct `_entry_door_style` at render time when the stored value lost side
 * information (e.g. backfill script defaulted to "single" without reading the
 * item.sides field or "Porte + N latéraux" type). Returns a style that exists
 * in ENTRY_DOOR_STYLES; falls back to the stored value.
 */
function resolveEntryDoorStyle(item) {
  const stored = item._entry_door_style;
  const sides = Number(item.sides) || 0;
  const type = (item.type || '').toLowerCase();
  const specsText = Array.isArray(item.specs)
    ? item.specs.join(' ').toLowerCase()
    : String(item.specs || '').toLowerCase();

  // Infer sides count from type/specs when not explicit on the item.
  let inferredSides = sides;
  if (!inferredSides) {
    const m = type.match(/(\d+)\s*lat[eé]rau/)
      || type.match(/porte\s*\+\s*(\d+)/)
      || specsText.match(/(\d+)\s*lat[eé]rau/);
    if (m) inferredSides = parseInt(m[1], 10) || 0;
  }

  // Double door hints — only upgrade if stored style doesn't already encode it.
  const looksDouble = /porte\s+double|\bdouble\s+door|double.*astragal|double.*post/.test(type + ' ' + specsText);

  // Guard: only upgrade "single" (the ambiguous default). Never downgrade a
  // style the devis tool explicitly selected.
  if (stored === 'single' || !stored) {
    if (inferredSides === 1) return 'single_1side';
    if (inferredSides >= 2) return 'single_2sides';
    if (looksDouble) return 'double_post';
  }
...
export function richItemSvg(item, width, height) {
  ...
  if (cat === 'entry_door' && item._entry_door_style) {
    return renderEntryDoorSvg(resolveEntryDoorStyle(item), width || 100, height || 110);
  }
