...
function colorTriad(item) {
  ...
  const int = item?.color_int || '';
  if (ext && int && ext.toLowerCase() !== int.toLowerCase()) {
    return { ext, int, combined: `ext. ${ext} / int. ${int}` };
  }
  const single = ext || int || item?.color_name || item?.color || '';
  return { ext: single, int: single, combined: single };
}

// ENERGY STAR policy — mirrors lib/quote-templates/pur.js:
//   - explicit opt-out (energy_star === false) wins
//   - otherwise: windows + patios default to true; doors default to false
//     unless explicit opt-in (energy_star === true)
function shouldShowEnergyStar(item, cat) {
  if (item?.energy_star === false || item?.energy === false) return false;
  if (item?.energy_star === true || item?.energy === true) return true;
  return cat === 'fenetre' || cat === 'patio';
}
...
function buildBdcSpecsLines(rawItem) {
  ...
  if (cat === 'fenetre') {
    ...
    // Line 3 — Couleurs ext/int (only if different)
...
export function buildBcHtml({ bc_number, supplier, date, projects, totalItems, totalQty, businessName, authorizedRep, hideSupplierName }) {
  ...
  const projectsHtml = projects.map(proj => {
    ...
    // BDC — it goes to the supplier, not the client. Just the project
    // reference is enough to associate the order downstream.
    const projHeader = `
