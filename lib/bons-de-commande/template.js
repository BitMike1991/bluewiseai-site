...
function buildBdcSpecsLines(rawItem) {
  ...
  // Épaisseur totale du cadre is ALWAYS the 2nd line — Mikael 2026-04-22:
  // "cest genre ultra important ca cest supposer etre le deuxieme specs".
  const frameThickness = pickForm(item, 'frame_thickness');
  const thicknessLine = frameThickness
    ? `Épaisseur totale du cadre : ${formatInches(frameThickness)}"`
    : null;
  ...
  if (cat === 'fenetre') {
    ...
    // Line 2 — Épaisseur totale du cadre (locked position per Mikael).
    if (thicknessLine) lines.push(thicknessLine);

    // Line 3 — Cadre / profilé with width + color
    ...
    // Line 4 — Couleur intérieure (always on fenêtre BDC, defaults Blanc).
...
function renderItemCardBc(item, idx) {
  ...
  const specsHtml = specsParts.length > 0
    ...
    : '';
  // Per-item note ("Évier", "Cuisine", "Porte arrière"...). Top-level note
  // is the source of truth; legacy line_items only carry it inside the specs
  // string as "Note: X". Falls back to explicit room/_room/location fields
  // if someone ever populates those.
  const noteFromSpecs = (() => {
    const raw = Array.isArray(item.specs) ? item.specs.join(', ') : String(item.specs || '');
    const m = raw.match(/(?:^|[,·])\s*Note\s*:\s*([^,·]+)/i);
    return m ? m[1].trim() : null;
  })();
  const room = item.note || noteFromSpecs || item.room || item._room || item.location || null;
  const roomHtml = room
  ...
  return `
    <div class="bc-item-card">
      ...
      <div class="bc-item-qty" title="Quantité">Qté : ${qty}</div>
...
export function buildBcHtml({ bc_number, supplier, date, projects, totalItems, totalQty, businessName, authorizedRep, hideSupplierName, projectNotesByRef }) {
  ...
  const projectsHtml = projects.map((proj, projIdx) => {
    ...
    // Global project notes — attached to the quote that produced these items.
    // Surfaced directly under the project header so the fournisseur sees
    // Jérémy's project-wide instructions (e.g. "Livrer à Saint-Sauveur le
    // mardi", "Cadres en vrac, pas assemblés").
    const projNotes = proj.project_notes
      || (projectNotesByRef && proj.project_ref ? projectNotesByRef[proj.project_ref] : null);
    const projNotesHtml = projNotes
      ? `<div class="bc-proj-notes"><strong>Notes du projet :</strong> ${escHtml(String(projNotes)).replace(/\n/g, '<br/>')}</div>`
      : '';
    const projHeader = `
...
<div class="page">
  ...
  <div class="bc-summary">
    ...
    <div class="summary-chip" title="Articles à produire (somme des quantités)">Articles à produire: <strong>${totalQty}</strong></div>
    <div class="summary-chip" title="Nombre de configurations uniques">Configurations: <strong>${totalItems}</strong></div>
  </div>
  ...
  <footer class="bc-footer">
    <div class="footer-totals">
      ...
      <div class="footer-total-item">
        <span class="footer-total-label">Articles à produire</span>
        <span class="footer-total-val">${totalQty}</span>
      </div>
      <div class="footer-total-item">
        <span class="footer-total-label">Configurations</span>
  ...
  for (const ref of itemRefs) {
    ...
    if (!byJob[key]) {
      byJob[key] = {
        ...
        // Global project notes from the source quote so the fournisseur
        // sees Jérémy's whole-project instructions, not just per-item ones.
        project_notes: q.notes || null,
        client_name: job?.client_name || null,
        client_address: job?.client_address || null,
        items: [],
      };
    } else if (!byJob[key].project_notes && q.notes) {
      byJob[key].project_notes = q.notes;
    }
