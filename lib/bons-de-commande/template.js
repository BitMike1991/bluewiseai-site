...
export function buildBcHtml({ bc_number, supplier, date, projects, totalItems, totalQty, businessName, authorizedRep, hideSupplierName }) {
  ...
  const projectsHtml = projects.map((proj, projIdx) => {
    ...
    // Mikael 2026-04-22: project ref must be SHORT — supplier won't hand-copy
    // "PUR-20260422-aBcDe" onto every product tag. BDC-local index "#1/#2/..."
    // is all they need to match items to sections.
    const shortTag = `#${projIdx + 1}`;
    ...
    const projHeader = `
      <div class="bc-proj-header">
        <div class="bc-proj-meta">
          <span class="bc-proj-number">Projet ${escHtml(shortTag)}</span>
        </div>
