...
function renderItemCardBc(item, idx) {
  ...
  const dims = (item.dimensions?.width && item.dimensions?.height)
    ...
    : '';
  // Sanitized specs: stripCoupeFroid each line, then dedupeSpecs to collapse
  // duplicate Thermos / Configuration / etc lines (Mikael 2026-04-21 PUR rules).
  const specsParts = cleanSpecs(item.specs);
