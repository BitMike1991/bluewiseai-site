...
function parseFrac(str) {
  ...
  // "69,5" → Quebec PDFs use comma as decimal separator. parseFloat drops
  // the comma part by default, silently under-pricing items. Handle it.
...
export function computeClientPrice(item, pricing, hardcodedConfig) {
  ...
  if (!Number.isFinite(listPrice) || listPrice <= 0) return { clientUnit: 0, clientTotal: 0, cost: 0, _perimeter: 0, _urethane: 0, _moulure: 0, _calking: 0 };
...
/**
 * Compute client unit price directly from a pre-discounted cost (Jérémy-entered
 * manual cost). Skips the escompte step — the cost passed in is what Jérémy
 * actually pays the supplier. Markup + linear + urethane + moulure + calking
 * + $400 floor all still apply, matching the canonical formula.
 *
 * Used by the manual cost input in DevisEditor so Jérémy can price an item
 * without the supplier PDF, by typing the cost he knows he'll pay.
 *
 * @param {Object} item    - { dimensions, qty }
 * @param {number} cost    - pre-discounted cost in CAD
 * @param {Object} pricing - DEFAULT_PRICING or customer override
 * @returns same shape as computeClientPrice
 */
export function computeClientPriceFromCost(item, cost, pricing = DEFAULT_PRICING) {
  const costNum = Number(cost);
  if (!Number.isFinite(costNum) || costNum <= 0) {
    return { clientUnit: 0, clientTotal: 0, cost: 0, _perimeter: 0, _urethane: 0, _moulure: 0, _calking: 0 };
  }
  // Delegate to computeClientPrice with escomptePct=0 and the already-discounted
  // cost as the list price. Preserves the single source of truth for markup +
  // surcharges + floor — no parallel math to drift.
  return computeClientPrice(
    { ...item, unitPrice: costNum },
...
export function computeProjectTotals(items, pricing = {}, opts = {}) {
  // "Petits frais" master toggle (default true). When false:
  //   - overhead + gaz set to 0
  //   - per-item cannette credit (urethane/moulure/calking already baked
  //     into each item's `total`) is subtracted from itemsTotal
  const petitsFrais = opts.petitsFrais !== false;

  const rawItemsTotal = (items || []).reduce((s, it) => s + (Number(it.total) || 0), 0);
  ...
  const itemsTotal = rawItemsTotal - cannetteCredit;

  const overhead   = petitsFrais ? Number(pricing.overheadPerJob ?? 200) : 0;
  const gaz        = petitsFrais ? Number(pricing.gazPerJob      ?? 100) : 0;
  const container  = opts.container ? 175 : 0;
  // Optional promo rebate (porte simple offerte — see lib/devis/promo.js).
  const promoRebate = Math.max(0, Number(opts.promoRebate || 0));
  const rawSubtotal = itemsTotal + overhead + gaz + container;
  ...
  return {
    ...
    cannetteCredit: Math.round(cannetteCredit * 100) / 100,
    overhead,
    gaz,
    container,
    petitsFrais,
    promoRebate: Math.round(promoRebate * 100) / 100,
