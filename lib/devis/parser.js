...
/**
 * Strip "Triple" and adjacent "coupe froid" if they appear as a pair.
 * Rule: ONLY remove "Triple" when paired with "coupe froid" (case-insensitive).
 * Standalone "coupe-froid Low-E" is preserved.
 */
function stripTriple(str) {
  if (!str || typeof str !== 'string') return str;
  // Remove "Triple coupe froid" (with/without hyphen, any case)
  let result = str.replace(/\bTriple\s+coupe[-\s]froid\b/gi, '');
  // Remove remaining bare "Triple" word (leftover or standalone)
  result = result.replace(/\bTriple\b/gi, '');
  // Clean up double commas / leading-trailing commas / extra whitespace
...
function parseSoumission(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (model && width && height) {
      items.push({ itemNumber, model, dimensions: { width, height }, qty, type: stripTriple(type), ouvrant, unitPrice, totalPrice });
    }
  }
