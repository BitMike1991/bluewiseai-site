...
export const SQFT_PER_PAQUET = 32;  // pi² couverts par un paquet de bardeau standard
...
export const MATERIALS = {
  ...
  bardeau_target_50: {
    id: 'bardeau_target_50',
    label: 'Bardeau Target 50 ans',
    price: 163.75,
    unit: 'paquet',
    sqft_per: 32,
    category: 'shingle_premium'
  },
  bardeau_target_100: {
    id: 'bardeau_target_100',
    label: 'Bardeau Target 100 ans',
    price: 100.76,
    unit: 'paquet',
    sqft_per: 32,
    category: 'shingle_premium'
...
export function calculateRoofingQuote(input) {
  const {
    ...
    days = 1,
    hours_per_day = LABOR.hours_per_day,
    pitch_category = 'easy',
    pitch_tier = 1,
    surface_sqft = 0,
    override_rate = null,
  ...
  // Priority: worker_rates explicit array > legacy (workers × hourly_rate).
  ...
  if (override_subtotal_ht !== null && override_subtotal_ht > 0) {
    ...
    rate_per_sqft = pitch_tier === 2 ? rate_cfg.tier2 : rate_cfg.tier1;
    revenue_ht = round2((rate_per_sqft * surface_sqft) - cash_discount);
  }
