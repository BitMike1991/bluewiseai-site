...
import { richItemSvg } from '../quote-templates/svg-ssr.js';
import { dedupeSpecs, renderDimsHtml } from '../devis/specs.js';
import { buildCondensedSpecsLines } from '../bons-de-commande/template.js';
...
function itemSketchSvg(item) {
  // LEGAL: the contract is what prevails in a dispute. The SVG here MUST
  // match what was on the devis the client saw. Delegate to the same rich
  // generator used by the quote template + editor so all three surfaces
  // (editor preview, /q/[token], contract HTML) render identical geometry.
...
function renderItemCard(item, index, complexityMul = 1) {
  ...
  // ENERGY STAR: same rule as devis + BDC (Mikael 2026-04-22 "le contrat
  // aussi devrait etre pareille"). ON by default on every line; only an
  // explicit energy_star === false / energy === false opts the line out.
  const energy = !(item.energy_star === false || item.energy === false);
  const cert = item.cert || null;
  // Per-item note — same legacy fallback as devis for line_items that only
  // carry it inside the specs string ("Note: Cuisine").
  const noteFromSpecs = (() => {
    const raw = Array.isArray(item.specs) ? item.specs.join(', ') : String(item.specs || '');
    const m = raw.match(/(?:^|[,·])\s*Note\s*:\s*([^,·]+)/i);
    return m ? m[1].trim() : null;
  })();
  const note = item.note || noteFromSpecs || null;
  ...
  // Mikael 2026-04-22: same condensed format as devis + BDC. Devis is the
  // signed source of truth; contract just re-renders its line_items through
  // the same condensation pipeline so the client sees identical item
  // identity at every step (devis → contrat → BDC). ENERGY STAR is filtered
  // out of the bullet list because it's already surfaced as the ★ pill.
  const condensed = buildCondensedSpecsLines(item)
    .filter((l) => !/^Certifié\s+ENERGY\s+STAR$/i.test(l));
  const specsHtml = condensed.length > 0
    ? `<ul class="specs">${condensed.map(s => `<li>${escHtml(s)}</li>`).join('')}</ul>`
    : '';
  ...
  // Apply complexity invisibly so per-item totals match the signed devis.
  // The contract re-renders from the quote's stored line_items (raw prices);
  // without this multiplier, per-item totals on the contract would NOT match
  // the per-item totals on the devis the client accepted.
  ...
  // Photos of the existing opening — same as the quote template, ported to
  // the contract so the install crew has the same legal reference attached.
  ...
  <div class="item-card">
    ...
    <div class="item-sketch">
      ${sketchSvg}
      ${dimsHtml}
      <div class="item-qty" title="Quantité">Qté : ${qty}</div>
      ${photosHtml}
    </div>
    <div class="item-info">
...
function renderWarrantyGrid(warranties) {
  // Mikael 2026-04-21 — never render a hardcoded fallback warranty grid; if
  // the tenant didn't seed quote_config.quote.warranties we show a single
  // generic line so we never leak old durations (e.g. "Installation 1 an")
  // when the operator wanted just "Installation 7 ans".
  if (!warranties || warranties.length === 0) {
    return `<div class="warranty-grid"><div class="warranty-card"><h4>Garanties</h4><ul><li><span class="w-item">Garantie conforme aux normes de l'industrie</span></li></ul></div></div>`;
  }
  const groups = {};
...
export function generatePurContractHtml(data, config) {
  ...
  // Jérémy's signature: PNG image preferred when configured, stylized
  // Dancing Script text as fallback so legacy tenants without an uploaded
  // signature still get the existing visual.
  const authorizedSignatureUrl = c.authorized_signature_url || '';
  const jeremySignatureHtml = authorizedSignatureUrl
    ? `<img src="${escHtml(authorizedSignatureUrl)}" alt="Signature ${escHtml(authorizedRep)}" style="display:inline-block;max-height:52px;max-width:220px;object-fit:contain;vertical-align:middle;"/>`
  ...
  // Mikael 2026-04-22: project_ref (PUR-0042) is THE visible number — same
  // on devis, contrat and BDC so the whole paper trail lines up. The raw
  // contract_number is a 16-hex security token (kept in the URL + DB for
  // unguessable access) and shown only as a tiny internal audit ref.
  const displayNum = project_ref || contract_number;
  const displayNumEsc = escHtml(displayNum);
  ...
  // Complexity multiplier from quote meta — must match the devis render
  const complexityMulStored = Number(meta?.complexity_multiplier);
  const complexityPct = Math.max(0, Number(meta?.complexity_pct || 0));
  const complexityMul = Number.isFinite(complexityMulStored) && complexityMulStored > 0
    ? complexityMulStored
    : 1 + complexityPct / 100;

  const sousTotalFenetres = fenItems.reduce((s, it) => s + (Number(it.total) || 0), 0) * complexityMul;
  const installationCost = installItem ? (Number(installItem.total) || 0) * complexityMul : 0;
  ...
  if (fenetreItems.length > 0) {
    ...
    itemsHtml += `<div class="item-grid">${fenetreItems.map((item, i) => renderItemCard(item, i + 1, complexityMul)).join('\n')}</div>`;
  }
  if (porteItems.length > 0) {
    ...
    itemsHtml += `<div class="item-grid">${porteItems.map((item, i) => renderItemCard(item, porteOffset + i + 1, complexityMul)).join('\n')}</div>`;
  }
  if (fenetreItems.length === 0 && porteItems.length === 0 && fenItems.length > 0) {
    itemsHtml += `<div class="item-grid">${fenItems.map((item, i) => renderItemCard(item, i + 1, complexityMul)).join('\n')}</div>`;
  }
  if (installItem) {
...
<head>
  ...
  <title>Contrat PÜR — ${displayNumEsc}</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Items always one above the other (Mikael 2026-04-21) — even desktop. */
    .item-grid { display: block; margin-bottom: 24px; }
    .item-grid > .item-card { margin-bottom: 16px; }
    .item-grid > .item-card:last-child { margin-bottom: 0; }
    ...
    .item-qty { display: inline-block; margin-top: 4px; background: var(--sage); color: var(--navy); font-family: 'JetBrains Mono', monospace; font-size: 10px; font-weight: 700; padding: 3px 10px; border-radius: 2px; text-align: center; white-space: nowrap; }
    ...
    @media (max-width: 768px) {
      ...
      .item-grid > .item-card { margin-bottom: 14px; }
    ...
    @media print {
      ...
      .signature-section:not(.signed) { display: none !important; }
      /* Switch grid → block so page-break-inside:avoid works reliably on each
         item (Chrome/Safari print ignores break-inside on grid items). */
      .item-grid {
      ...
      .item-sketch { width: 100px; gap: 4px; }
      .item-sketch svg { max-width: 88px; max-height: 88px; }
      .item-sketch .dims { font-size: 9px; white-space: nowrap; }
      .item-type { font-size: 12px; margin-bottom: 6px; line-height: 1.25; }
      .specs { margin: 0 0 6px 0; }
      .specs li { font-size: 10px; line-height: 1.4; padding-left: 8px; }
      .specs li::before { top: 7px; }
      .energy { font-size: 8px; padding: 1px 6px; margin-top: 4px; }
      .contrat-clause { break-inside: avoid; page-break-inside: avoid; }
      .scope-table { break-inside: auto; }
      .scope-table tr { break-inside: avoid; }
      .payment-list, .warranty-grid { break-inside: auto; }
      .payment-item, .warranty-card { break-inside: avoid; page-break-inside: avoid; }
      .section-title { break-after: avoid; break-inside: avoid; }
      /* Same fix as quote: coord cards (Parties au contrat) split mid-card on
         portrait Letter when the second card flows onto a new page. Force
         block + avoid. */
      .coord-grid { display: block !important; gap: 0; }
      .coord-card {
        break-inside: avoid;
        page-break-inside: avoid;
        margin-bottom: 12px;
      }
      /* Signature section: when signed, keep entire block on one page so the
         signed-by + date + ip line never orphan. */
      .signature-section.signed { break-inside: avoid; page-break-inside: avoid; }
      /* Section-level: allow internal breaks but never orphan the title. */
      .section { break-inside: auto; }
      a[href^="http"]:after { content: " (" attr(href) ")"; font-size: 9px; opacity: 0.6; }
    }
...
<div class="watermark">CONFIDENTIEL — CONTRAT ${displayNumEsc}</div>

<button class="print-fab no-print" onclick="window.print()" aria-label="Imprimer ce contrat">
  🖨️ Imprimer / PDF
</button>
...
<div class="page">
  ...
  <div class="meta-bar">
    <div class="cell">
      ...
      <div class="value">${displayNumEsc}</div>
    </div>
    <div class="cell">
  ...
  <section class="section">
    ...
    <div class="projet-box">
      ...
      ${project_notes ? `<p style="margin-top:8px;"><strong>Notes du projet :</strong> ${escHtml(project_notes).replace(/\n/g, '<br/>')}</p>` : ''}
  ...
  <section class="section" style="page-break-before:always;">
    <div style="background:var(--navy);color:var(--pure);padding:28px 32px;border-radius:4px;margin-bottom:24px;">
      ...
      <div style="font-size:11px;opacity:0.55;margin-top:6px;">Contrat n° ${displayNumEsc} · ${clientNameEsc}</div>
    </div>
    ...
    <div style="margin-top:32px;padding-top:24px;border-top:1px solid var(--border);">
      <p style="font-size:12px;"><strong>La présente Annexe A fait partie intégrante du Contrat n° ${displayNumEsc} et a la même force obligatoire.</strong></p>
...
<div class="signature-section" id="signatureSection">
  ...
  <div class="sig-checkbox">
    ...
    <label for="acceptTerms">
      J'ai lu et j'accepte l'ensemble des termes et conditions du contrat n° ${displayNumEsc}, incluant l'Annexe A — Étendue des travaux. Je reconnais que cette signature électronique a la même valeur juridique qu'une signature manuscrite.
    </label>
  </div>
