...
export async function runGetKpisTool(supabase, customerId, args) {
  ...
  // ── Build rich summary ──
  const lines = [
    `📊 BUSINESS PERFORMANCE (${periodLabel})`,
    ``,
    `LEADS: ${totalLeads}${leadTrend !== null ? ` (${leadTrend >= 0 ? "+" : ""}${leadTrend}% vs previous period)` : ""}`,
    `  By status: ${Object.entries(statusCounts).map(([s, c]) => `${s}: ${c}`).join(", ")}`,
    `  Top sources: ${topSources.map(([s, c]) => `${s}: ${c}`).join(", ") || "none"}`,
    ``,
    `CONVERSION FUNNEL:`,
    `  ${totalLeads} leads → ${allQuotes.length} quotes ($${Math.round(quotesValue)}) → ${signedJobs.length} signed → ${allPayments.length} payments`,
    `  Conversion rate: ${conversionRate}%`,
    ``,
    `REVENUE: $${Math.round(totalRevenue).toLocaleString()}${revTrend !== null ? ` (${revTrend >= 0 ? "+" : ""}${revTrend}% vs previous period)` : ""}`,
    `  Pipeline value: $${Math.round(pipelineValue).toLocaleString()}`,
    `  Won value: $${Math.round(wonValue).toLocaleString()}`,
    ``,
    `COMMUNICATIONS: ${allMessages.length} messages`,
    `  ${Object.entries(msgByChannel).map(([ch, c]) => `${ch}: ${c}`).join(", ")}`,
    `  Inbound: ${inbound} | Outbound: ${outbound}`,
    ``,
    `CALLS: ${allCalls.length} total, ${successfulCalls} successful, ${meetingsBooked} meetings booked${avgCallScore !== null ? `, avg score: ${avgCallScore}` : ""}`,
    ``,
