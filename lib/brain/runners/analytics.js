...
export async function runGetKpisTool(supabase, customerId, args) {
  ...
  const summaryParts = [];
  summaryParts.push(`[INSTRUCTION: The KPI cards are already shown to the user visually. Your job is to ANALYZE this data like a business co-founder. Write 4-6 sentences: headline insight, what's working, what's concerning, and 1-2 actionable recommendations. Be direct and opinionated. DO NOT list numbers — they can see the cards.]`);
  summaryParts.push(``);
  summaryParts.push(`DATA FOR YOUR ANALYSIS:`);
  ...
  summaryParts.push(`Status breakdown: ${Object.entries(statusCounts).map(([s, c]) => `${s}: ${c}`).join(", ")}.`);
  summaryParts.push(`Funnel: ${totalLeads} leads → ${allQuotes.length} quotes ($${Math.round(quotesValue).toLocaleString()}) → ${signedJobs.length} signed → ${allPayments.length} payments. Conversion: ${conversionRate}%.`);
  summaryParts.push(`Revenue: $${Math.round(totalRevenue).toLocaleString()}. Pipeline: $${Math.round(pipelineValue).toLocaleString()}. Won: $${Math.round(wonValue).toLocaleString()}.`);
