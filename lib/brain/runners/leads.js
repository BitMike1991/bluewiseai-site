...
export async function runFindLeadTool(supabase, customerId, args) {
  ...
  if (!candidates || candidates.length === 0) {
    ...
    if (effectiveName && typeof effectiveName === "string" && effectiveName.trim().length > 0) {
      ...
      // Fallback 1: try each word separately
      ...
      // Fallback 2: trigram similarity (catches typos like Mathieu/Matthieu)
      if (candidates.length === 0) {
        matchReason = "name_trigram";
        const { data: triData, error: triErr } = await supabase
          .rpc("search_leads_fuzzy", {
            p_customer_id: customerId,
            p_search: effectiveName.trim(),
            p_limit: max,
          });

        if (!triErr && triData && triData.length > 0) {
          candidates = triData;
        }
      }
    } else {
