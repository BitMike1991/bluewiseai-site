...
`.trim();
  ...
  for (let i = 0; i < v; i++) {
    const variantStyle =
      i === 0
        ? "Direct and action-oriented. Propose a specific time. Example: 'Salut X! Jérémy peut te voir lundi 19h, ça marche?'"
        : i === 1
        ? "Warm check-in. Ask about their project first, then suggest next step. Example: 'Hey X, ton projet avance bien? On peut se voir cette semaine si tu veux.'"
        : "Ultra-short nudge. Under 60 characters. Example: 'Salut X, toujours intéressé?'";

    const completion = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      temperature: i === 0 ? 0.3 : 0.8,
      messages: [
        {
          role: "system",
          content:
