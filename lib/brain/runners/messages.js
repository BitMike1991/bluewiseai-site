...
`.trim();
  ...
  for (let i = 0; i < v; i++) {
    const variantStyle =
      i === 0
        ? "Direct — get straight to the point in as few words as possible"
        : i === 1
        ? "Warm — friendly and casual, like texting a friend about business"
        : "Ultra-short — absolute minimum words, telegram style";

    const completion = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      temperature: i === 0 ? 0.3 : 0.5,
      messages: [
        {
          role: "system",
          content:
