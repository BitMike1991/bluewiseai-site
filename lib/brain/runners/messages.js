...
`.trim();
  ...
  for (let i = 0; i < v; i++) {
    const variantStyle =
      i === 0
        ? "CLOSER approach — assume the sale, propose a specific time, make it easy to say yes. Go straight for the booking. Example (FR): 'Salut Matthieu! Jérémy peut te voir lundi 19h, on confirme?'"
        : i === 1
        ? "NURTURE approach — check in on their project, show interest in their needs, gently suggest next step. No pressure. Example (FR): 'Hey Matthieu, comment ça avance ton projet de fenêtres? Si t'as des questions Jérémy peut t'aider.'"
        : "FOMO approach — create urgency with availability or promotion. Example (FR): 'Matthieu, on a un créneau qui vient de se libérer cette semaine. Tu veux le prendre?'";

    const completion = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      temperature: i === 0 ? 0.3 : 0.75,
      messages: [
        {
          role: "system",
          content:
