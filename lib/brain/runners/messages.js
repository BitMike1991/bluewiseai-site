...
// Lazy-init to prevent module-scope crash when OPENAI_API_KEY is not yet available
let _openai = null;
function getOpenAI() {
  if (!_openai) {
...
export async function runDraftReplyTool(supabase, customerId, args) {
  ...
  const v = Math.min(Math.max(Number(variants) || 3, 1), 3);

  const structuredSystem =
    sendChannel === "sms"
      ? 'Output STRICT JSON only: { "body": "..." }. No markdown, no backticks.'
      : `Output STRICT JSON only: { "subject": "...", "body": "..." }. No markdown, no backticks.`.trim();

  const smsExamples = `
<examples>
GOOD SMS (natural, short, one question):
- "Hey Matthieu! Toujours partant pour les fenêtres? Jérémy peut te voir lundi soir, 19h ça marche?"
- "Salut Marc, juste un suivi — t'as eu le temps de regarder la soumission?"
- "Hi Sarah, quick follow-up on your window project. Still interested? We have openings this week."

BAD SMS (too long, too formal, multiple questions):
- "Bonjour Matthieu! Je suis ravi que tu sois toujours intéressé par ton projet de fenêtres et porte d'entrée. Rencontrer Jérémy te permettra d'obtenir des conseils personnalisés pour avancer rapidement. Nous avons des créneaux lundi soir..." (TOO LONG, sounds corporate)
</examples>`;

  const userPrompt = `
${structuredSystem}

<task>Draft a ${sendChannel.toUpperCase()} in ${langLine}.</task>

<style>
${sendChannel === "sms" ? `- MAX 160 characters. One sentence, one question. Like a real person texting.
- Sound human, not corporate. Use "tu" in French, casual tone.
- ONE call-to-action only. Ask one thing, wait for the answer.
- If a follow-up time exists, confirm it simply. If not, propose one specific time.
${smsExamples}` : `- Subject: short, specific, action-oriented.
- Body: 3-5 sentences max. Warm but professional.
- One clear CTA at the end.`}
</style>

<constraints>
- NEVER write more than 2 sentences for SMS.
- NEVER sound like a marketing email or corporate template.
...
`.trim();
  ...
  const variantStyles = [
    { style: "CLOSER approach — assume the sale, propose a specific time, make it easy to say yes. Go straight for the booking. Example (FR): 'Salut Matthieu! Jérémy peut te voir lundi 19h, on confirme?'", temp: 0.3 },
    { style: "NURTURE approach — check in on their project, show interest in their needs, gently suggest next step. No pressure. Example (FR): 'Hey Matthieu, comment ça avance ton projet de fenêtres? Si t'as des questions Jérémy peut t'aider.'", temp: 0.75 },
    { style: "FOMO approach — create urgency with availability or promotion. Example (FR): 'Matthieu, on a un créneau qui vient de se libérer cette semaine. Tu veux le prendre?'", temp: 0.9 },
  ].slice(0, v);

  // Generate all variants in parallel for speed
  const completions = await Promise.all(
    variantStyles.map(({ style, temp }) =>
        ...
        messages: [
          {
            role: "system",
            content:
              `You write ${sendChannel === "sms" ? "text messages" : "emails"} for a trades contractor CRM. ` +
              `Sound like a real person, not a bot. Keep it SHORT. ` +
              `IMPORTANT: Write ONLY in ${langLine}. Never mix languages. ` +
              `Style: ${style}.`,
          },
  ...
  for (let i = 0; i < completions.length; i++) {
    ...
    if (!body) continue; // skip failed variants
