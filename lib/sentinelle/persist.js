...
export async function persistRun(checks, results) {
  ...
  // last_error_payload tracks the most recent diagnostic across non-ok runs so
  // alerts + dashboard can show "why is this check unhealthy right now?" without
  // paginating through events.
  const resultsById = new Map(results.map((r) => [r.id, r]));
  await Promise.all(checkUpserts.map(async (u) => {
    const r = resultsById.get(u.id);
    const patch = {
      ...
      last_error_payload: r?.error_payload || null,
    };
