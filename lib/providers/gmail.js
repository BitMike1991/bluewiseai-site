...
/**
 * RFC 2047 encoded-word for non-ASCII header text. Without this, Gmail emits
 * raw UTF-8 bytes in From/Subject headers — clients render them as Latin-1
 * mojibake ("PÃfÂœR" instead of "PÜR", "Ã¢Â€Â"" instead of "—").
 */
function encodeHeaderWord(str) {
  if (str == null) return "";
  const s = String(str);
  if (/^[\x00-\x7F]*$/.test(s)) return s; // all ASCII → leave as-is
  return `=?UTF-8?B?${Buffer.from(s, "utf-8").toString("base64")}?=`;
}

/**
 * Encode a "Display Name <email>" header. Only the display name is encoded;
 * the email address itself must stay raw ASCII per RFC 5322.
 */
function encodeAddressHeader(addr) {
  if (!addr) return "";
  const s = String(addr).trim();
  const m = s.match(/^(.*?)\s*<([^<>]+)>\s*$/);
  if (!m) return s; // bare email, no display name
  const name = m[1].trim().replace(/^["']|["']$/g, "");
  const email = m[2].trim();
  if (!name) return `<${email}>`;
  const encoded = encodeHeaderWord(name);
  // Wrap ASCII names in quotes when they contain reserved chars; encoded-words
  // are already safe unquoted.
...
function buildMimeMessage({ from, to, subject, body, html, replyTo }) {
  ...
  let mime = "";
  mime += `From: ${encodeAddressHeader(from)}\r\n`;
  mime += `To: ${encodeAddressHeader(to)}\r\n`;
  mime += `Subject: ${encodeHeaderWord(subject)}\r\n`;
  if (replyTo) mime += `Reply-To: ${encodeAddressHeader(replyTo)}\r\n`;
  ...
  // Base64url encode the full UTF-8 message (Gmail API requires base64url).
