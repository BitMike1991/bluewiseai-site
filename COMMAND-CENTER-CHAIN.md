...
- ~~NEVER ship P02 without closing the **PUR overhead gap**~~ — **RESOLVED in P01.6 commit 59e8685 via prorata scaling** (Mikael's call: "splitter prorata par item comme le reste"). Mapper now scales each `line.unit_price` by `subtotal / sum(line_items.total)` so the displayed invoice mirrors the BW quote 1:1 — no synthetic "Frais généraux" line. Verified: quote 64 → Invoice 146, qbo_total 8071 = bw_subtotal 8071, checksum_match: true.
- NEVER send a QBO `Line` where `Amount ≠ Qty × UnitPrice` — QBO error 6070 ("Amount calculation incorrect") with zero cent tolerance. Always compute UnitPrice first (one rounding to 2 decimals), then DERIVE Amount = Qty × UnitPrice. Absorb prorata drift by nudging the LAST line's UnitPrice (not Amount) so the invariant holds. See lib/qbo/invoiceMapper.js L53-L98.
